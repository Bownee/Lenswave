...
internal object ExifOrientation {
    fun apply(source: Bitmap, orientation: Int, filter: Boolean): Bitmap {
    ...
    fun degrees(orientation: Int): Int = when (orientation) {
    ...
    fun orientedToRaw(rect: Rect, orientation: Int, rawWidth: Int, rawHeight: Int): Rect = when (orientation) {
        ...
        ExifInterface.ORIENTATION_TRANSPOSE -> Rect(rect.top, rect.left, rect.bottom, rect.right)
        ExifInterface.ORIENTATION_ROTATE_90 ->
    ...
    fun rawToOriented(rect: Rect, orientation: Int, rawWidth: Int, rawHeight: Int): Rect = when (orientation) {
        ExifInterface.ORIENTATION_FLIP_HORIZONTAL ->
            Rect(rawWidth - rect.right, rect.top, rawWidth - rect.left, rect.bottom)
        ExifInterface.ORIENTATION_ROTATE_180 ->
            Rect(rawWidth - rect.right, rawHeight - rect.bottom, rawWidth - rect.left, rawHeight - rect.top)
    ...
    private fun matrix(orientation: Int): Matrix? = Matrix().apply {
        when (orientation) {
