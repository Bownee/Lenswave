...
internal data class AvailableUpdate(val versionName: String)

@Singleton
class AppUpdateChecker @Inject internal constructor(
    ...
    internal suspend fun findAvailableUpdate(currentVersionName: String): AvailableUpdate? = try {
    ...
    private fun refresh(state: AppUpdateState, nowMillis: Long): AppUpdateState = when (
        val result = client.fetch(state.etag)
    ) {
        is LatestReleaseResult.Modified -> state.copy(
            latestVersionName = result.versionName,
            etag = result.etag,
            nextCheckAtMillis = nowMillis + SUCCESS_INTERVAL_MILLIS,
        )
        LatestReleaseResult.NotModified -> state.copy(
            nextCheckAtMillis = nowMillis + SUCCESS_INTERVAL_MILLIS,
        )
        LatestReleaseResult.Unavailable -> state.copy(
