...
# Exit 1 when anything was flagged. Only members at the class's own indentation are
# considered, so locals inside functions and nested classes do not count.
status=0
for f in "$@"; do
  awk -v f="$f" '
    ...
    seeninit && indent($0) == member && /^ *(private |internal |protected |public )?(lateinit )?(val|var) [A-Za-z_][A-Za-z0-9_]* *(:|=)/ {
